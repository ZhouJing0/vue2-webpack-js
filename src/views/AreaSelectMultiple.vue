  ...
  <div class="manyAreaSelect">
    ...
    <div class="item">
      ...
      <div class="item-footer">
        <button
          ...
          :disabled="!selectItem.province.regionName"
        >
          不限城市
        </button>
      </div>
    ...
    <div class="item">
      ...
      <div class="item-content">
        ...
        <ul
          v-show="
            !notLimitButton.notLimitCity &&
            !notLimitButton.notLimitDistrict &&
            districList.length !== 0
          "
        >
          <li
            v-for="(item, index) in districList"
            :class="item.selected ? 'active' : ''"
            :key="index"
            @click="getAreaCombineID(item)"
          >
            {{ item.regionName }}
          </li>
        </ul>
      ...
      <div class="item-footer">
        <button
          ...
          :disabled="
            !selectItem.city.regionName ||
            !selectItem.province.regionName ||
            notLimitButton.notLimitCity
          "
        >
          不限地区
      ...
      <div class="trangle-wrap">
        <div class="left">
          <button
            ...
            :disabled="direactionStatusToRight"
          >
        ...
        <div class="right">
          <button
            ...
            :disabled="direactionStatusToLeft"
          >
    ...
    <div class="item">
      ...
      <div class="item-footer">
        <button
          ...
          :class="selectedAllButtonStatus ? 'success' : ''"
        >
          {{ selectedAllButtonStatus ? "反选" : "全部" }}
        </button>
      </div>
    </div>
  </div>
...
import chinaArea from "./DLJSON/chinaArea.json";
export default {
  name: "manyAreaSelect",
  data: function () {
    return {
  ...
  computed: {
    ...
    direactionStatusToRight() {
      // 控制可以转移的箭头状态
      if (
        this.notLimitButton.notLimitCity ||
        this.notLimitButton.notLimitDistrict
      ) {
  ...
  methods: {
    ...
    getAreaCombineID(item) {
      ...
      if (item) {
        ...
        if (item.selected) {
          this.leftData.push({
            regionName:
              this.selectItem.province.regionName +
              "-" +
              this.selectItem.city.regionName +
              "-" +
              item.regionName,
            regionId:
              this.selectItem.province.regionId +
              "-" +
    ...
    districNotLitmit(item) {
      ...
      this.leftData = []; // 请空数组
      this.notLimitButton.notLimitDistrict =
        !this.notLimitButton.notLimitDistrict; // 不限按钮状态
      this.leftData.push({
        regionName:
          this.selectItem.province.regionName +
          "-" +
