<template>
  <div class="easyframe-table-box" :class="[config.__config__.type]">
    <div class="easyframe-common-title" v-if="config.__config__.showTitle && config.__config__.label">
      <h2>{{ config.__config__.label }}</h2>
    </div>
    <el-table
      :data="tableFormData"
      class="easyframe-common-table"
    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          ...
          <template v-if="['select', 'checkbox', 'radio'].includes(head.__config__.easyframeKey)">
            <el-select
              v-model="tableFormData[scope.$index][cindex].value"
              v-bind="getConfById(head.__config__.formId)"
              :rowIndex="scope.$index"
              @change="onFormDataChange(scope.$index, cindex, 'el-select')"
            >
              <el-option
                v-for="(opt, oindex) in head.__slot__.options"
                :key="oindex"
          ...
          <!-- 单行输入 -->
          <template v-else-if="head.__config__.easyframeKey === 'comInput'">
            <el-input
              v-model="tableFormData[scope.$index][cindex].value"
              v-bind="getConfById(head.__config__.formId)"
              :rowIndex="scope.$index"
  ...
  methods: {
    buildOptions() {
      this.tableData.forEach(cur => {
        ...
        if (dyOptionsList.indexOf(config.easyframeKey) > -1) {
          let isTreeSelect = config.easyframeKey === 'treeSelect' || config.easyframeKey === 'cascader'
    ...
    onFormDataChange(rowIndex, colIndex, tag) {
      ...
      if (['easyframe-Amount', 'el-input-number'].includes(tag)) {
        // 金额变动 更新数据 触发计算公式更新
