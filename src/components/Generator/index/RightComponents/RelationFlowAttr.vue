<template>
  <el-row>
    ...
    <el-form-item label="关联字段">
      <el-select v-model="activeData.showField" placeholder="请选择关联字段" @visible-change="visibleChange" clearable>
        <el-option v-for="item in fieldOptions" :key="item.vmodel" :label="item.label" :value="item.vmodel" />
      </el-select>
    </el-form-item>
  </el-row>
</template>
    ...
    getOptions() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__config__ && data.__config__.easyframeKey) {
          if (data.__config__.easyframeKey === 'relationFlow' && data.__vModel__) {
            list.push(data)
          }
        }
      }
