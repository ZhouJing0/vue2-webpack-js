<template>
  <el-row>
    ...
    <el-dialog
      ...
      width="600px"
      class="easyframe-dialog easyframe-dialog_center"
      @open="onOpen"
      append-to-body
    >
      <div class="calc-box">
...
export default {
  ...
  computed: {
    calculateCmps() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__config__ && data.__config__.easyframeKey) {
          if (['numInput', 'calculate'].includes(data.__config__.easyframeKey)) {
            const isTableChild = parent && parent.__config__ && parent.__config__.easyframeKey === 'table'
            calcList.push({
  ...
  methods: {
    reloadExpressionTemp() {
      const isValid = d => {
        const target = this.calculateCmps.find(
          cmp => cmp.__vModel__ === d.__vModel__ && cmp.__vModel__ === d.__vModel__
        )
