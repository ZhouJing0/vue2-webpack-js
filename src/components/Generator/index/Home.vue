  ...
  <div class="container">
    ...
    <div class="center-board">
      <div class="action-bar">
        <el-button icon="el-icon-video-play" type="text" @click="preview" size="medium"> 预览</el-button>
    ...
    />
    <json-drawer size="550px" :visible.sync="jsonDrawerVisible" :jsonData="formData" @refresh="refreshJson" />
...
import { exportDefault, beautifierConf, isNumberStr, titleCase, deepClone } from '@/components/Generator/utils'
...
export default {
  ...
  created() {
    ...
    if (this.modelType == 1 || this.modelType == 6)
      this.leftComponents[1].list = [...this.leftComponents[1].list, ...webPeculiarList]
  },
  mounted() {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (
              this.$store.getters.hasTable &&
              config.layout === 'colFormItem' &&
              !noVModelList.includes(config.easyframeKey) &&
              !e.__vModel__
            ) {
              reject({ msg: `请选择${config.label}的字段名`, target: 1 })
              break
            }
            if (config.easyframeKey === 'billRule') {
              if (!config.rule) {
            ...
            if (config.easyframeKey === 'relationForm') {
              if (!e.modelId) {
            ...
            if (config.easyframeKey === 'relationFlow' && !e.flowId) {
    ...
    shouldClone(to, from, target, event, conf) {
      ...
      if (conf.__config__.rowType === 'table') {
        if (noTableAllowList.includes(targetConf.__config__.easyframeKey)) {
          // if (this.showTip) {
    ...
    createIdAndKey(item) {
      ...
      if (config.layout === 'colFormItem') {
        if (!this.$store.getters.hasTable) {
          // 分割线和按钮不加vModel
          if (noVModelList.indexOf(config.easyframeKey) > -1) return
          item.__vModel__ = `${config.easyframeKey}Field${this.idGlobal}`
        } else {
          if (noVModelList.indexOf(config.easyframeKey) > -1) return
          item.__vModel__ = ''
        }
      } else if (config.layout === 'rowFormItem') {
        if (config.easyframeKey === 'table') {
          item.__vModel__ = `${config.easyframeKey}Field${this.idGlobal}`
        }
