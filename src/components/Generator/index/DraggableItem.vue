...
const layouts = {
  ...
  rowFormItem(h, element, index, parent) {
    ...
    if (element.__config__.easyframeKey === 'row') {
    ...
    if (element.__config__.easyframeKey === 'table') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
          ...
          >
            ...
            <el-form label-position="top">
              <draggable
                ...
                animation={340}
                group={group}
                class="drag-wrapper table-wrapper table-wrapper-web"
                onEnd={onEnd}
    ...
    if (element.__config__.easyframeKey === 'card') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
