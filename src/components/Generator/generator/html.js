...
const tags = {
  ...
  'easyframe-UploadImg': function (el) {
  ...
  'easyframe-UploadFz': function (el) {
    const tag = this['easyframe-UploadImg'](el)
    const buttonText = `buttonText="${el.buttonText}"`
    return addPropToTag(tag, buttonText)
  },
  'easyframe-Quill': function (el) {
    const { tag, vModel, placeholder } = attrBuilder(el)
