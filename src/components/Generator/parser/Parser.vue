...
const layouts = {
        ...
        <el-col span={config.span}>
          ...
          >
            <render
              ...
              conf={scheme}
              {...{on: listeners}}
              ref={config.rowType === 'table' ? scheme.__vModel__ : undefined}
            />
          </el-form-item>
        </el-col>
      )
  ...
  rowFormItem(h, scheme) {
    ...
    if (scheme.__config__.easyframeKey === 'row') {
      if (scheme.type === 'flex') {
        child = (
...
export default {
  ...
  methods: {
    initFormData(componentList = [], formData) {
      console.log(componentList, formData, 333333)
      this.$store.commit('generator/UPDATE_RELATION_DATA', {})
      componentList.forEach(cur => {
        const config = cur.__config__
        if (cur.__vModel__) formData[cur.__vModel__] = config.defaultValue
        if (cur.__config__.easyframeKey == 'table') return
    ...
    buildOptions(componentList, data) {
      componentList.forEach(cur => {
        ...
        if (dyOptionsList.indexOf(config.easyframeKey) > -1) {
          let isTreeSelect =
            config.easyframeKey === 'treeSelect' || config.easyframeKey === 'cascader'
        ...
        if (config.children && config.easyframeKey !== 'table')
          this.buildOptions(config.children, data)
      })
    },
    ...
    async submitForm() {
      ...
      const valid = await this.$refs[this.formConf.formRef].validate()
