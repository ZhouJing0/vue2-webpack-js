  ...
  <div>
    <template v-if="formData.popupType === 'general'">
      <el-dialog
        ...
        append-to-body
      >
        <div class="dynamicDetail" v-loading="loading || mainLoading" :element-loading-text="$t('common.loadingText')">
          <Parser :formConf="formData" :relationData="relationData" @toDetail="toDetail" v-if="!loading" />
        </div>
    ...
    <template v-if="formData.popupType === 'fullScreen'">
      <transition name="el-zoom-in-center">
        <div class="JNPF-preview-main">
          ...
          <div
            class="dynamic-form-main dynamicDetail"
      ...
      for (let i = 0; i < realList.length; i++) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (
              list[i].__config__ &&
              list[i].__config__.jnpfKey !== 'table' &&
              list[i].__config__.children &&
              Array.isArray(list[i].__config__.children)
            ) {
