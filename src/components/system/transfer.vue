...
export default {
  ...
  methods: {
    ...
    onCheck(data, checked) {
      ...
      if (this.type) {
        if (this.type == "customAudit") {
          this.selectedTextData = [data];
        } else {
          const list = this.$refs.treeBox.getCheckedNodes(true);
          this.selectedTextData = list;
          // this.selectedTextData = list.filter(o => o.type == this.type)
        }
      } else {
        this.selectedTextData = this.$refs.treeBox
        ...
        console.log(this.selectedTextData);
      }
