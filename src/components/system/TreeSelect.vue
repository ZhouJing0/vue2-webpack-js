...
export default {
  ...
  methods: {
    ...
    setSelectOption(node) {
      const tmpMap = {};
      tmpMap.value = node.data[this.props.value];
      tmpMap.label = node.label;
      this.options = [];
      this.options.push(tmpMap);
      console.log(node);
      this.selectedData = node.data[this.props.value];
    },
    // 单选，选中传进来的节点
    ...
    // 多选，节点勾选状态发生变化时的回调
    handleCheckChange() {
      var checkedKeys = this.$refs.tree.getCheckedKeys(true, true); // 所有被选中的节点的 key 所组成的数组数据 这里两个true，1. 是否只是叶子节点 2. 是否包含半选节点（就是使得选择的时候不包含父节点）
